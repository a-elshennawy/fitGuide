...
const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        ...
        cb(null, `squat_workout_${timestamp}.webm`);
    }
});
...
function cleanWorkoutData(workoutData) {
    if (workoutData.sessionData) {
        delete workoutData.sessionData.frame_count;
        delete workoutData.sessionData.knee_angles;
        delete workoutData.sessionData.knee_ratios;
        delete workoutData.sessionData.rep_knee_angles;
        delete workoutData.sessionData.rep_markers;
    }
    return workoutData;
}
...
app.post('/api/save-workout', async (req, res) => {
    try {
        ...
        try {
            ...
            history = history.map(entry => cleanWorkoutData(entry));
        } catch (error) {
