    ...
    if (poses && poses.length > 0) {
      ...
      if (pose.keypoints && pose.keypoints.length > 0) {
        ...
        if (elbowAngle < ELBOW_ANGLE_UP_THRESHOLD + 20) {
          console.log(
            "Calibration successful: User is in starting position with arms bent"
          );
          return true;
        } else {
          feedbackHistory.set("Get in position (arms bent)", {
            color: "#FFA500",
...
function processDetection(keypoints) {
  ...
  if (visibleLandmarks.length >= requiredLandmarks.length - 1) {
    detectionFound = true;
    const leftVisibility =
      keypoints[5].score + keypoints[7].score + keypoints[9].score;
    const rightVisibility =
      keypoints[6].score + keypoints[8].score + keypoints[10].score;
    ...
    const knee =
      keypoints[keypointMap[useRightSide ? "right_knee" : "left_knee"]];
    const ankle =
      keypoints[keypointMap[useRightSide ? "right_ankle" : "left_ankle"]];
    ...
    let elbowVerticalAngle =
      mag1 * mag2 === 0
        ? 0
        : Math.acos(Math.min(1, Math.max(-1, dot / (mag1 * mag2)))) *
          (180 / Math.PI);
    if (
      (useRightSide && shoulder.x < elbow.x) ||
      (!useRightSide && shoulder.x > elbow.x)
    ) {
    ...
    if (exerciseState === "counting") {
      ...
      if (backAngle > BACK_ANGLE_MAX) {
        ...
        const forwardPoint = {
          x: midback.x + (useRightSide ? 50 : -50),
          y: midback.y + 30,
        };
        drawArrow(ctx, midback, forwardPoint, "#FF0000", 4);
        feedbackHistory.set("Bend your back slightly forward", {
          color: "#FF0000",
          count: 2,
        });
        ...
        feedbackHistory.set("Straighten your back a bit", {
          color: "#FF0000",
          count: 2,
        });
      ...
      if (repState === "up" && elbowAngle > ELBOW_ANGLE_DOWN_THRESHOLD) {
        repState = "down";
        console.log(
          `State change: up -> down (angle: ${elbowAngle.toFixed(1)})`
        );
        ...
        if (
          ...
          if (hasError && !currentRepHasError) {
            ...
            if (currentRepErrors.includes("Back too straight")) {
              badReps.back_too_straight++;
            }
            if (currentRepErrors.includes("Back too tilted")) {
              badReps.back_too_tilted++;
            }
          ...
          // Check if we've reached total reps
        ...
        console.log(
          `State change: down -> up (angle: ${elbowAngle.toFixed(1)})`
        );
      }
...
function showSessionComplete() {
  ...
  // Get the session complete screen element
  ...
  // Update form issues
  document.getElementById("elbow-swinging").textContent =
    badReps.elbow_swinging || 0;
  document.getElementById("back-too-straight").textContent =
    badReps.back_too_straight || 0;
  document.getElementById("back-too-tilted").textContent =
    badReps.back_too_tilted || 0;
  ...
  // Show saving video status
  const videoStatus = sessionComplete.querySelector(".video-status");
  ...
  const progressInterval = setInterval(() => {
    ...
    if (progress >= 100) {
      ...
      console.log("Session complete screen shown");
    }
  }, 100);
  ...
  if (!sessionData.saved) {
    ...
    try {
      const response = await fetch(
        "http://localhost:3001/api/save-tricep-workout",
        {
          method: "POST",
          headers: {
      ...
      if (result.success) {
        ...
        // Show session complete screen with the workout data
        const completionData = {
          ...
          formIssues: {
            ...
            backTooTilted: badReps.back_too_tilted || 0,
            backTooStraight: badReps.back_too_straight || 0,
          },
          performanceScore:
            Math.round(((counter - badReps.total) / counter) * 100) || 0,
        };
...
function calculatePerformanceScore() {
  ...
  return Math.round(((counter - badReps.total) / counter) * 100);
}
